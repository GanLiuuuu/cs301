...
void KEY_Init(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;				 //蜂鸣器PA8
...
void keysacn()
{
  int val;
  val=KEY;  //读取按键
  while(!GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_4))//当按键没有被按下时，一直循环
  {
    val=KEY;//词句可省略，可让循环跑空
  }
  while(GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_4))//当按键被按下时
  {
    delay_ms(10);	  //延时10ms
    val=KEY;//读取数字端口PC4口电平值赋给Val
    if(val==1)  //第二次判断按键是否被按下
    {
      BEEP_SET;		//蜂鸣器响
      while(!GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_4))	//判断按时是否被松开
        BEEP_RESET;	//蜂鸣器停止
    }
    else
				BEEP_RESET;//蜂鸣器停止
